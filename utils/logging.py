...
def create_log_directory(
    base_dir: Union[str, Path], variable: Optional[str] = None
) -> Path:
...
def get_log_file_path(
    base_dir: Union[str, Path], variable: str, year: Optional[int] = None
) -> Path:
...
def setup_variable_logging(
    ...
    if console_only:
        ...
        logger.info(
            f"Console-only logging configured for {variable}"
        ...
        try:
            ...
            logger = get_logger(__name__)
            logger.info(
                f"Logging configured for {variable}" + (f" year {year}" if year else "")
            )
