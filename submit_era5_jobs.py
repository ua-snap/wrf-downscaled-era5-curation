...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--variables", type=str, help="Comma-separated list of variables to process"
    )
    parser.add_argument(
        "--start_year",
        type=int,
    ...
    def wait_for_available_slots():
        ...
        while True:
            try:
                ...
                logger.info(
                    f"Currently {current_jobs} jobs in queue, waiting for some to complete..."
                )
                time.sleep(15)
            except subprocess.CalledProcessError:
                logger.warning(
                    "Failed to check current job count, proceeding with submission"
                )
                break
...
def wipe_slurm_output_logs(
    base_log_dir: Path = Path.cwd() / "logs" / "era5_process",
) -> None:
...
def wipe_application_logs(
    base_log_dir: Path = Path.cwd() / "logs", logger=None
) -> None:
    ...
    while True:
        try:
            ...
            if current_time - last_log_time >= 300:
                ...
                logger.info(
                    f"Still waiting for {current_jobs} jobs to complete (elapsed: {elapsed_minutes:.1f} minutes)"
                )
...
def submit_timeout_retries(
    timeout_jobs: List[Tuple[str, int]], overwrite: bool = False
) -> int:
    ...
    for variable, year in timeout_jobs:
        try:
            ...
            logger.error(
                f"RETRY: Failed to submit retry job for {variable} year {year}: {e}"
            )
            logger.error(f"stdout: {e.stdout}")
            logger.error(f"stderr: {e.stderr}")
            continue
    ...
    logger.info(
        f"Successfully submitted {retry_count} retry jobs out of {len(timeout_jobs)} timeouts"
    )
    return retry_count


def validate_job_completion(
    variables: List[str], start_year: int, end_year: int
) -> None:
...
def main() -> None:
    ...
    setup_variable_logging(
        variable="submit_era5_jobs", base_dir=Path.cwd(), verbose=False
    )
