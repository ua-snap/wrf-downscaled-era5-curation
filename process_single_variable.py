...
def get_year_filepaths(year: int) -> List[Path]:
    ...
    if not fps:
        raise FileNotFoundError(
            f"No files found for year {year} with pattern {file_pattern}"
        )
...
def read_data(
    ...
    with xr.open_dataset(filepaths[0]) as sample_ds:
        ...
        original_chunks = {
            dim: sample_ds[dim].shape[0]
            for dim in sample_ds.dims
...
def process_files_in_batches(
    ...
    logger.info(
        f"Processing {num_files} files in {num_batches} batches of {config.BATCH_SIZE}"
    )
    ...
    for i in range(num_batches):
        ...
        logger.info(
            f"Processing batch {i+1}/{num_batches} with {len(batch_files)} files"
        )
...
def write_output(ds: xr.Dataset, variable: str, output_file: Path) -> None:
    ...
    # gotta rename Time to time, of course
...
def process_variable_for_year(
    variable: str, year: int, overwrite: bool = False
) -> Path:
...
def main() -> None:
    ...
    setup_variable_logging(
        variable=args.variable, year=args.year, base_dir=Path.cwd(), console_only=True
    )

    try:
        # Process the variable
        result = process_variable_for_year(
            variable=args.variable, year=args.year, overwrite=args.overwrite
        )
